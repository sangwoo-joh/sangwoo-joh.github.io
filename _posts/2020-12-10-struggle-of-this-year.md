...
### 용량 길들이기
 ...
 받아둔 이미지가 그대로 있다. 덕분에 실험 서버를 포맷하거나 도커를
 재설치하는 삽질을 피할 수 있었다.
...
### 빌드 재현하기
 ...
 - GitHub 브랜치 정책으로 인해 PR이 merge 되자마자 head가 자동
   삭제되어 사라졌거나 -> 분석 시점에서 PR의 merge 여부를 체크해서
   있는게 보장된 브랜치를 활용하도록 하고,
 - PR head가 private fork 라서 권한이 없거나 -> 일단 PR의 base에는
   권한이 있기 때문에 여기 정보를 활용하도록 하고,
 - 스냅샷에서 분석한 브랜치가 outdated 되었고, 실제로 그 브랜치의
   빌드가 깨지거나 -> 브랜치가 아니라 커밋 아이디를 기준으로 빌드
   시도하도록 하고,
 - (빌드 정보만으로는 알 수 없는) 각종 권한 이슈로 빌드가 안되거나 ->
   로그를 뜯고 디버깅을 해서 개발팀에 직접 물어봐야 하고,
 - 서브 모듈 클론해와야 하는데 방화벽에 막혀있거나 -> 방화벽
   설정해주고,
 - 서브 모듈 클론하는데 ssh known_hosts에 등록이 안되어 있어서
   interactive 에 막힌 채로 멈춰버리거나 -> 호스트의 ssh 설정 물고
   들어가고,
