...
1. 일단 **디자인이 예뻐야 한다**. 보기 좋은 떡이 맛도 좋은 법. (내가
   보기에) 못생긴 UI에서 작업하고 싶지 않다. 이 분야 최고봉은
   개인적으로는 애플이라고 생각한다.
2. 드라이버 지원이 잘 되면 좋겠다. 일일이 하드웨어 알아내서 호환되는
   드라이버 설치하고 ... 사실 하려면 할 수 있고 또 해봤던 일이긴 한데,
   이제 귀찮다. 하지만 엔비디아는 내가 찾아야 겠지.
3. 안정적이면 좋겠다. 여기에는 두 가지가 내포되어 있는데, 사용하면서
   원인 모를 이유로 발생하는 크래시가 아주 드물 정도로 테스트가 잘 된
   OS면 좋겠고, 설령 크래시가 나더라도 든든한 개발진이 뒷받쳐줘서 다음
   릴리즈에 잘 수정되면 좋겠다.
4. 쓰기 편하면 좋겠다. 여기에도 두 가지가 내포되어 있는데, 이미 내
 ...
 그럼 나는 그동안 어떤 삽질을 해왔을까? 각 리눅스 배포판에 대한
 ...
 - [Ubuntu Gnome](https://ubuntugnome.org/),
   ...
   리눅스 배포판이 어떻게 꾸려지는지, 커널과 DE의 관계, 메인스트림인
   Gnome과 KDE 등을 알게 되었고, 바로 둘 다 시도했다. 사실 KDE의
   디자인이 더 마음에 들었어서 쿠분투를 주력으로 쓸 생각이었지만,
   그당시 KDE는 내 기준 매우 불안정한 상태였고 창 조절을 하다가 커널
   패닉이 나거나 글자가 삐져나오거나 하는 등의 이슈가 있어서 Gnome으로
   안착했다. 우분투도 결국 유니티를 버리고 Gnome으로 돌아왔으니
   어찌보면 잘 선택했던듯.
 - [Xfce](https://www.xfce.org/): 얘는 분명 써봤던 기억은 있는데 어떤
   배포판에서 쓴 건지 기억이 안난다. 주분투를 설치한 것은 아니었던 것
   같은데, 그냥 DE만 따로 깔아서 시도해봤었던가? 윈도우 95 테마가
   끌려서 시도했다가 금방 질려서 다시 돌아왔던 것 같기도 하고.
 - [Ubuntu MATE](https://ubuntu-mate.org): 끈닷넷님의 최애
   배포판이었던 기억이 있다. 옆에서 사용하는 걸 보니 내 취향은
   아니라서 보는 걸로만 만족하기로 했다. 사족으로 나는 여전히 마테가
   아니라 메이트로 읽힌다 😅.
 - [Ubuntu Netbook
 ...
 - [elementaryOS](https://elementary.io/): 어느 날 방문한
   ...
   버전이 1.대도 아닌 0.대의 Freya 였으니, 최신인 5.대의 Hera는 좀
   괜찮을려나? 궁금하긴 하지만 써보고 싶진 않다.

 이렇게 정리하고 보니 생각보다 많은 시간을 낭비해온 것
 같다(...). 아무튼 여러가지를 써보면서 지금은 데비안과 그놈 환경에
 정착하였다.

 사실 그동안의 배포판 떠돌이 생활은 1번 조건인 "심미적인(Aesthetic)
 디자인"이 만족되지 않았던 게 가장 크다. 그래서 그동안 애플 및 유사
 애플을 써보려고 많은 시도를 해왔다. 애플에 대해서 잘 모르던 시절에
 돈은 쓰기 싫고 해서 해킨토시를 한번 시도해볼까? 했지만 이건 2번과 3번
 조건을 매우 위반하는 일이라 당연히 시도조차 하지 않았다. 그래서 돈이
 없던 시절에는 애플의 디자인 철학을 따르는 몇몇 리눅스 배포판을
 시도해보기도 했다. 하지만 이런 배포판들은 1번 조건 외에는 잘 지켜지지
 않았고, 결국 항상 개발 머신에는 2, 3, 4번을 만족하는 우분투가 깔려
 있었다.

 어찌저찌 맥북을 사용하게 되면서 1번은 정말 만족스러웠지만, 생각보다
 나머지 부분에서 만족감이 쉽지 않았다. 2번이야, 애플은 지들이 만든
...
## 만자로 + 벗지
 ...
 - 마침 검색하다가 [이런 글](https://cozelsil.kr/407?category=96043)을
   ...
   떠오르던 차에,
 - 같은 사람이 쓴 [이 글](https://cozelsil.kr/418)과 유튜브 검색
   결과로 보게 된 [이
   영상](https://www.youtube.com/watch?v=UGHTAYMX8T8)에서 이쁜데
   안정적이기까지 한 Gnome 기반의 DE인 Budgie(이하 벗지)를 간접
   경험하면서,
 ...
 여기까지 대략 3시간에 걸친 삽질 끝에 만자로 벗지 1차 시도는
 실패하였다. 역시 리눅스 데스크탑의 길은 멀고도 험난하다.

 하지만 잠깐 사용해본 소감으로는,
 - `pacman`의 속도가 매우 훌륭하다. 처음에는 미러가 미국으로 되어
   있어서 다운로드 속도가 굉장히 느렸는데, 한국 미러로 바꾸고 나니
   이게 정말 롤링 업데이트가 맞나? 싶을 정도로 굉장히 속도가 빨랐다.
 - 벗지도 매우 훌륭하다. Gnome과 KDE에 대한 고찰 글을 쓰신 야관문
 ...
 직접 사용해볼 수 있는 좋은 경험이었다. 둘 다 매우 내 취향에
 맞다. 그래서 다음 업무 환경을 꾸릴 때에는 [Ubuntu
