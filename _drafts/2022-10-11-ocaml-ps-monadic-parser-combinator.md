...
---
 ...
 글이었다.  그때부터 kipa00님이 글로 남긴 이 문제는 언젠가 꼭 풀고
 싶다는 생각을 해왔다. 다만 같은 방법으로 풀면 재미없으니까 다른
 방법으로 풀어봐야지 라는 생각만 어렴풋이 가지고 있었다.
n
 하지만 정말로 이 문제를 풀기까지의 과정은 쉽지 않았다. 근본적으로는
 내가 PS 자체에 익숙하지 않은 탓이 컸다. 애초에 PS를 제대로 해본 적이
 없으니 당연한 일이었다. 데이터 구조와 알고리즘을 그냥 "알고" 있는
 것과, 온라인 저지 사이트에서 문제를 이해하고 풀이에 필요한 것들을
 코드로 모델링하고 시간 및 메모리 제한에 맞는 알고리즘을 구현해서 평가
 테스트 셋을 통과하는 것은 꽤 간극이 컸다. PS는 정말로 훈련이 필요한
 일이었다.
...
### 표현식 평가하기 (Evaluation)
 ...
 진리값 조합의 경우의 수가 $$ 2 ^ {10} = 1024 $$로 작기 때문에, 변수에
 대입할 수 있는 가능한 모든 진리값의 조합을 일일이 대입해서 두 식을
 평가한 다음 같은지 확인하면 된다. (문제 설명의 정규화는 반쯤은
 낚시다...)

 변수를 입력 그대로 `a-z` 값으로 가지고 있으면 변수에 진리값을
...
### 컴비네이터
 ...
 그리고 컴비네이터 패턴에는 너무 당연하게도(?) 모나드가 엄청나게
 쓰이는데 (...) 그래서 아예 Monadic Parser Combinator 라고 부르기도
 한다. 근데 일단 나부터가 모나드에 대해서 잘 설명할 자신이 없고, 또
 여기서 모나드가 무엇인지를 설명하기 시작하면 글이 안드로메다로
 가버리기 때문에... 모나드에 대해서는 그냥 짧게 내가 이해한 바만
 정리하자면, 어떤 *귀찮고 복잡한 처리*에 **반복적으로** 쓰이는 패턴을
 문법적으로 깔끔하게 처리하기 위한 디자인 패턴이다. 여기서 귀찮고
 복잡한 처리에는 파이프라이닝, 사이드 이펙트, 보일러플레이트 등이
 있다. 더 자세한 것은 위키피디아의
...
### 파서
 ...
 파서 컴비네이터의 문맥에서 파서란 문자열을 입력으로 받아서 해석한
 다음 (최소) *두 가지* 값을 리턴하는 **함수**를 뜻한다. 하나는 파서가
 파싱한 결과값 (e.g., 파싱 트리) 이고, 다른 하나는 파서가 **파싱을
 완료하고 남은 입력**의 정보이다. 컴비네이터는 이 남은 입력 정보를
 활용한다. 기본적인 (프리미티브) 파서들을 컴비네이터로 조합할 때, 어떤
 파서의 작업이 끝난 뒤 다음 파서가 작업할 위치를 이 정보로부터 알 수
 있다.

## 구현

 파서 컴비네이터를 글로 정리하긴 했지만, 개인적으로는 직접 구현하면서
 훨씬 더 많이 이해 할 수 있었다. 특히
 [Angstrom](https://github.com/inhabitedtype/angstrom/)과
 [Parcoom](https://github.com/tsoding/parcoom) 코드를 읽으면서 많이
 배웠다. 여기서 구현한 코드는 순전히 이 문제를 풀기 위한 것이므로
 프로덕션 레벨의 구현이 궁금하다면
...
### 프리미티브 파서
 ...
 이제 이후 정의될 (모나딕) 파서 컴비네이터를 가지고 복잡한 파서를
 만들기 위한 재료로 쓰일 기본적인 파서 두 개를 정의할 수 있다.
...
```
 ...
 컴비네이터들을 살펴보았다. 이제 필수적이면서도 아름답기까지 한 (?)
...
```
 ...
 식을 생성자로 묶어야 한다. 이 과정을 다른 언어로 구현했다면 스택을
 이용해서 구현했겠지만, 파서 컴비네이터로는 좀더 깔끔하게 할 수 있다.
 ...
 2. 왼쪽항 다음의 중위 연산자와 표현식 하나를 파싱한다.
    ...
    2.1. 파싱에 성공한 경우, 중위 연산자의 생성자로 이때까지 파싱한
       결과를 묶는다 (a). 그리고 다시 2.로 돌아가서 중위 연산자와 표현식
       하나를 계속해서 파싱한다 (b).
 ...
 이 과정은 파싱하고자 하는 대상이 조금만 복잡해져도 필수적인
 성질이다. 딱히 이름이 있는 것 같진 않지만 [Angstrom에서는
...
```
 ...
 표현식을 파싱하는데 이 값은 가장 왼쪽항이 되므로 중위 연산 체이닝의
 초기항이라고 볼 수 있다. 따라서 이 값을 `>>=` 에 넘겨주면서
 `init`으로 네이밍했다. 그리고 이 초기값으로부터 중위연산자를 만날
 때마다 계속해서 그에 맞는 생성자 (`constructor`) 를 호출해서
 누적하도록 한다. 그리고 이 리프팅의 결과를 가지고 다시 `further`를
 호출해서 앞에서 말한 과정을 반복하거나, 혹은 반복에 실패 (정확히는
 리프팅의 첫 번째 파서인 `op`, 즉 연산자 파싱에 실패) 한 경우 이때까지
 쌓은 결과값을 곧바로 리턴한다 (`further <|> return acc`). 미학이
 느껴지는 코드다.

 `lift2`에 넘겨주는 파서의 순서에 주목하자. `init`에서 이미 왼쪽항의
 표현식을 파싱했으므로 그 다음은 중위 연산자 `op`를 파싱하고 그 다음
 표현식 `expr`을 파싱해야 한다. 그리고 리프팅 함수에 있는 생성자
 (`constroctur`) 가 바로 위에서 정의한 이항 연산자 파서들 `and_`,
 `or_`, `xor`이 파싱한 결과 `and_of`, `or_of`, `xor_of`인데, 이전에
 파싱해서 누적된 `acc`를 먼저 전달해줘야 순서가 맞는다. 예를 들어 `a +
 b + c`를 체이닝하게 되면 `((a + b) + c)`로 묶여야 하는 것과
...
```
 ...
 왜냐하면 `Angstrom`에 더 일반적인 멋진 해결책이 있었기
 때문이다. 재귀적인 데이터를 파싱한다고 해서 무한히 파싱 가능한 것은
 아니다. 파싱한 결과를 계속 쌓아나간다고 하면 어느 시점에는 결과가 더
 이상 쌓이지 않는 시점이 올텐데, 이를 보통 Fixed Point라고 하고 이를
 계산해주는 컴비네이터를 [Fixed Point
 ...
 정확히 이 역할을 하는데, 파서의 Fixed Point를 계산해주는 파서를
 만들어주는 컴비네이터이다.
...
```
 ...
 `fix (fun r -> ...)`는 `r`을 재귀적으로 적용할 수 있는 파서를
 만들어준다. 코드를 보면 `r`은 `fix (fun r -> ...)`와 같다는 것을 알
 수 있다. `r`을 호출하지 않고 파싱에 성공한 경우는 더 이상 재귀호출이
 없으므로 종료된다.
...
### 표현식 파서 (드디어)
 ...
 이제 우리 손에 있는 고급 파서들과 체이닝, Fixed point 컴비네이터를
 이용하면 드디어 정말로 표현식 파서를 만들 수 있을 것 같다. 나이브하게
 다음과 같이 구현할 수 있다.
...
```
 ...
 것이다. BNF에서 논터미널의 심볼의 깊이가 깊을수록 우선순위가 높다.
...
```
 ...
 이제 남은 일은 표현식 파서에 줄 입력을 다듬고 문제 형태에 맞게
 출력하는 것이다. 문제의 조건에 따르면 입력 표현식은 공백도
 포함하는데, 우리는 공백을 파싱하는 파서는 따로 만들지
 않았다. 괄호와는 달리 공백은 표현식의 의미에 영향을 끼치지 않아서
 그냥 전처리로 공백을 없애버리는 것이 훨씬 쉽기 때문이다. 그러므로
 공백을 없애는 `remove_blanks`를 만들자.
...
```
 ...
 구현했다면 두 표현식을 구분하는 일은 아주 쉽다: 그냥 두 번 파싱하면
 된다. 같은 파서를 두 번 적용해서 튜플로 만들어주는 컴비네이터
 `pair`를 이용하면 좋을 것 같다.
